...
func defaultSettings() userSettings {
	// Everything at defaults (no override file), and auto-skip off — the
	// long-standing behaviour.
	return userSettings{Sections: map[string]sectionDelta{}, SkipOnFailure: false, CleanWorkAfterBuild: false}
}
...
func (us userSettings) settingsFromEnabled(enabled map[string]bool) userSettings {
	res := userSettings{Sections: map[string]sectionDelta{}, SkipOnFailure: us.SkipOnFailure, CleanWorkAfterBuild: us.CleanWorkAfterBuild, ForceAndroidDocker: us.ForceAndroidDocker}
	for _, sec := range toggleSections() {
		var d sectionDelta
		for _, it := range sec.items() {
			if it.Required {
...
// _audio_only.sh sources this file, so the re-exported values win.
func (us userSettings) writeOverride(scriptsRoot string) error {
	...
	b.WriteString("# Sourced by scripts/shared/_audio_only.sh.\n\n")
